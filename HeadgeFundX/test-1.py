...
for times in range(2, 3):
    ...
    plt.xlabel('epochs')
    # plt.show()
    plt.savefig('{0:%m%d%H%M%S}'.format(datetime.datetime.now())+'acc_hiddenlayers'+str(times)+'.png')
    ax_acc = fig.add_subplot(111)
    ax_acc.plot(range(epochs), val_acc, label='acc', color='black')
    ax_loss = ax_acc.twinx()
    ax_loss.plot(range(epochs), val_loss, label='loss', color='gray')
    plt.xlabel('epochs')
    plt.savefig('{0:%m%d%H%M%S}'.format(datetime.datetime.now())+'acc-loss_hiddenlayers_'+str(times)+'.png')
...
for (acc, loss) in zip(result_acc, result_loss):
    for (i, j) in zip(acc, loss):
        w_list.append([i, j])
    w_list.append(['##################', '##################'])
with open('{0:%m%d%H%M%S}'.format(datetime.datetime.now())+'result.csv', 'a', newline='') as file:
